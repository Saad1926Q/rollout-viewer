...
export default function RolloutViewer() {
  ...
  const selected = run?.rollouts.find((rollout) => rollout.id === selectedId);
  ...
  const loadTrajectory = async (file: File) => {
    try { const parsed = parseRolloutFile(await readFile(file), file.name); setRun(parsed); setSelectedId(parsed.rollouts[0]?.id ?? ""); setError(""); }
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      const index = filtered.findIndex((rollout) => rollout.id === selectedId);
      if (event.key === "j" || event.key === "ArrowDown") { event.preventDefault(); filtered[index + 1] && setSelectedId(filtered[index + 1].id); }
      if (event.key === "k" || event.key === "ArrowUp") { event.preventDefault(); filtered[index - 1] && setSelectedId(filtered[index - 1].id); }
    };
