...
import { allRollouts, formatNumber, parseRunContent, type JsonObject, type JsonValue, type Message, type Rollout, type Run, type Turn } from "../lib/rollout";
...
function RunSummary({ run }: { run: Run }) {
  return <main className="transcript"><div className="transcript-heading"><div><p className="eyebrow">run overview</p><h1>Run summary</h1><p>{run.groups.length} groups · {allRollouts(run).length} rollouts</p></div></div><DataPanel title="summary" data={run.summary} /><DataPanel title="run metadata" data={run.metadata} /></main>;
}
...
export default function RolloutViewer() {
  ...
  const statuses = useMemo(
    () => [...new Set(rollouts.flatMap((rollout) => rollout.status ? [rollout.status] : []))].sort(),
    [rollouts],
  );
  const filteredGroups = useMemo(
    () => run?.groups.map((group) => ({
      ...group,
      rollouts: group.rollouts.filter((rollout) =>
        (filter === "all" || rollout.status === filter)
        && `${rollout.id} ${JSON.stringify(rollout.raw)}`.toLowerCase().includes(query.toLowerCase()),
      ),
    })).filter((group) => group.rollouts.length > 0) ?? [],
    [run, filter, query],
  );
  const visible = filteredGroups.flatMap((group) => group.rollouts);
  const selected = rollouts.find((rollout) => rollout.key === selectedKey);

  const loadRollouts = async (file: File) => {
    try {
      const parsed = parseRunContent(await readFile(file), file.name);
      const loaded = allRollouts(parsed);
      setRun(parsed);
      setSelectedKey(loaded[0]?.key ?? "");
      setShowSummary(false);
      setError("");
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Could not read that rollout file.");
    }
  };
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      const index = visible.findIndex((rollout) => rollout.key === selectedKey);
      if ((event.key === "j" || event.key === "ArrowDown") && visible[index + 1]) {
        event.preventDefault();
        setShowSummary(false);
        setSelectedKey(visible[index + 1].key);
      }
      if ((event.key === "k" || event.key === "ArrowUp") && visible[index - 1]) {
        event.preventDefault();
        setShowSummary(false);
        setSelectedKey(visible[index - 1].key);
      }
  ...
  if (!run) {
    return <div className="landing"><div className="landing-title"><span>rollout viewer</span><p>Read the run. Find the failure. Keep moving.</p></div><UploadCard onLoad={loadRollouts} onError={setError} />{error && <p className="error">{error}</p>}<p className="landing-note">All parsing happens locally. Unknown fields remain available as structured turn data.</p></div>;
  }
  ...
  return <div className="viewer-shell"><header className="run-header"><div><p className="eyebrow">{run.format}</p><h1>{String(run.metadata.name ?? run.metadata.model ?? run.sourceName)}</h1><p>{run.groups.length} groups · {rollouts.length} rollouts</p></div><div className="run-stats"><span><b>{run.groups.length}</b> groups</span><span><b>{formatNumber(averageReward)}</b> avg reward</span><span><b>{rollouts.length}</b> rollouts</span><button className={showSummary ? "summary-button active" : "summary-button"} onClick={() => setShowSummary(true)}>run summary</button><button className="summary-button" onClick={() => { setRun(null); setSelectedKey(""); setFilter("all"); setQuery(""); setShowSummary(false); setError(""); }}>home</button></div></header>{error && <p className="error app-error">{error}</p>}<aside className="sidebar"><div className="sidebar-controls"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find a task or rollout" aria-label="Find a task or rollout" /><select value={filter} onChange={(event) => setFilter(event.target.value)} aria-label="Filter rollouts"><option value="all">all results</option>{statuses.map((status) => <option key={status} value={status}>{label(status)}</option>)}</select></div><p className="result-count">{visible.length} of {rollouts.length} rollouts · <kbd>j</kbd> <kbd>k</kbd> to move</p><nav>{filteredGroups.map((group) => <section className="group" key={group.id}><h2>{group.label}</h2>{group.rollouts.map((rollout) => <button className={rollout.key === selectedKey ? "rollout-row selected" : "rollout-row"} onClick={() => { setShowSummary(false); setSelectedKey(rollout.key); }} key={rollout.key}><span className="row-title">rollout {rollout.index}</span>{rollout.status && <span className="status">{label(rollout.status)}</span>}<span className="row-meta">reward {formatNumber(rollout.reward)} · {rollout.turns.length} turns</span></button>)}</section>)}</nav></aside>{showSummary ? <RunSummary run={run} /> : selected ? <Transcript rollout={selected} /> : <main className="empty-selection">No rollout matches this filter.</main>}</div>;
}
